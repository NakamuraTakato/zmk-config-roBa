...
- **レイヤー4**: MOUSE（マウスクリック）- **Mac/Win共通**
- **レイヤー5**: SCROLL（スクロールモード）- **Mac/Win共通**
- **レイヤー6**: 設定レイヤー（共通）
...
**注**: レイヤー4（MOUSE）とレイヤー5（SCROLL）はMac/Windows共通です。マウスクリックやスクロール操作はOS依存の動作がないため、どちらのOSでも同じレイヤーを使用します。トラックボールの自動レイヤー切り替え機能（`automouse-layer`）は1つのレイヤーしか指定できないため、この設計が最適です。
